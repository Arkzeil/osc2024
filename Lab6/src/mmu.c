...
// map a single page of the user space to a physical address
void map_one_page(size_t *virt_pgd_p, size_t va, size_t pa, size_t flag)
{
    ...
    for (int level = 0; level < 4; level++)
    {
        ...
        {
            table_p[idx] = pa;
            uart_sendline("map_one_page: VA: 0x%x, PA: 0x%x\r\n", va, pa);
            table_p[idx] |= PD_ACCESS | PD_TABLE | (MAIR_IDX_NORMAL_NOCACHE << 2) | PD_KNX | flag;
            return;
        }

        // the entry is not present in the page table
        if(!table_p[idx])
        {
        ...
        // next-level page table, and physical address stored in bits 47 to 12 of the entry.
...
void mmu_add_vma(struct thread *t, size_t va, size_t size, size_t pa, size_t rwx, int is_alloced)
{
    ...
    uart_sendline("vma added: va: 0x%x, pa: 0x%x, size: 0x%x, rwx: 0x%x, is_alloced: %d\r\n", va, pa, size, rwx, is_alloced);
}
...
void mmu_memfail_abort_handle(esr_el1_t* esr_el1)
{
    ...
    {
        uart_sendline("[Translation fault]: 0x%x %d\r\n",far_el1, curr_thread->pid);
